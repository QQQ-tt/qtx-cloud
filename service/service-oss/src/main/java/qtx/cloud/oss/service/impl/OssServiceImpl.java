...
public class OssServiceImpl implements OssService {
  ...
  public boolean upload(
    ...
    return saveFileInfo(
        otherInfo, business, version, oldFileName, newFileName, uploadFile, fileUuid);
  }
  ...
  public boolean uploads(
    ...
    for (MultipartFile file : files) {
      ...
      flag =
          flag
              & saveFileInfo(
                  otherInfo, business, version, fileName, newFileName, uploadFile, fileUuid);
    }
    return flag;
  }

  @Override
  public boolean uploadByBucket(
      MultipartFile file,
      String otherInfo,
      String bucket,
      String business,
  ...
  @Override
  public void downloadFileByBucket(String bucket, String fileObject, HttpServletResponse response)
      throws IOException {
  ...
  public boolean removeFile(String fileObject) {
    ...
    return service.remove(
        Wrappers.lambdaUpdate(FileManagement.class)
  ...
  @Override
  public void downloadFileBatch(HttpServletResponse response, List<OssBatchDTO> fileObjects)
      throws IOException {
    // 被压缩文件InputStream
    ...
    response.setHeader(
        "Content-Disposition", "attachment;filename=" + URLEncoder.encode("文件集合.zip", "UTF-8"));
  ...
  public void getFileName(
      String fileName, HttpServletResponse response, InputStream fileInputStream)
      throws IOException {
    FileManagement one =
        service.getOne(
            Wrappers.lambdaQuery(FileManagement.class).eq(FileManagement::getFileObject, fileName));
    String fileNameOld = one.getFileNameOld();
    response.setHeader(
        "Content-Disposition",
        "attachment;filename="
            + URLEncoder.encode(StringUtils.substringBefore(fileNameOld, "."), "UTF-8")
            + "."
